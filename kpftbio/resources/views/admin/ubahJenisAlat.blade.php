...
@section('content')
  ...
  <form action="{{ route('simpanUbahJenisAlat') }}" method="post" enctype="multipart/form-data">
      ...
      <table class="table table-striped-columns">
        ...
        <tbody id="spesifikasi-body">
          @foreach ($dataSpesifikasi as $spesifikasi)
            <tr draggable='true'>
              <td><input name='namaSpesifikasi[]' class="form-control" value="{{ $spesifikasi->nama }}" required /></td>
              <td><input name='spesifikasi[]' class="form-control" value="{{ $spesifikasi->spesifikasi }}" required /></td>
      ...
      <table class="table table-striped">
        ...
        <tbody id="sop-body">
          @foreach ($dataSop as $sop)
            <tr draggable='true'>
              <td><input name='sop[]' class="form-control" value="{{ $sop->sop }}" required /></td>
              <td><button class="btn btn-danger" type='button' onClick='delete_row(this)'>Hapus</button></td>
            </tr>
          @endforeach
        </tbody>
    ...
    <button class="btn btn-primary" type="submit" onclick="return confirm('Apakah data jenis alat sudah benar?')">Ubah
      Data Alat</button>
  </form>
  ...
  <form action="{{ route('simpanListAlat') }}" method="POST">
    ...
    <table class="table table-striped-columns">
      ...
      <tbody>
        @foreach ($dataAlat as $alat)
          ...
          <tr>
            <td href="{{ url('detailAlat/' . $alat->id) }}">{{ $dataJenisAlat->nama }} - {{ $alat->nomor }}
            </td>
            <td>
              <select class="form-select" id="listLabAlat" name="listLabAlat[]" required>
                @foreach ($dataLab as $lab)
            ...
            <td><a href="{{ url('hapusAlat/' . $alat->id) }}" class="btn btn-danger" onclick="return confirm('Hapus alat ({{ $dataJenisAlat->nama }} - {{ $alat->nomor }}) dan seluruh datanya?')">Hapus</a>
            </td>
    ...
    <br>
    <button class="btn btn-primary" type="submit" onclick="return confirm('Apakah daftar alat sudah benar?')">Ubah
      Daftar Alat</button>
  </form>
    ...
    $(document).ready(function() {
      ...
      $("#addSpesifikasi").click(function() {
        ...
        $("#spesifikasi-body").append(
          "<tr draggable='true'><td><input name='namaSpesifikasi[]' class='form-control' required /></td><td><input name='spesifikasi[]' class='form-control' required /></td><td><button type='button' class='btn btn-danger' onClick='delete_row(this)'>Hapus</button></td></tr>"
        );
      });
      ...
      $("#addSop").click(function() {
        var countItems = document.getElementById("spesifikasi-body").childElementCount;
        $("#sop-body").append(
          "<tr draggable='true'><td><input name='sop[]' class='form-control' required /></td><td><button type='button' class='btn btn-danger' onClick='delete_row(this)'>Hapus</button></td></tr>"
        );
      });
    });
